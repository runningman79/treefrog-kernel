/*
 * Xylon pixel clock generation logiCLK IP core
 *
...
#define MULT_STEPS       64
#define DIV_INPUT_STEPS  56
#define DIV_OUTPUT_STEPS 128
	...
	u32 *p_mult, u32 *p_div_in)
{
	...
	*p_div_in = 0;
	...
	for (din = 1; din <= DIV_INPUT_STEPS; din++) {
		for (mult = 2; mult <= MULT_STEPS; mult++) {
			for (dout = 1; dout <= DIV_OUTPUT_STEPS; dout++) {
				freq_hz = osc_clk_freq;
				freq_hz *= mult;
				freq_hz = div_u64_rem(freq_hz, din,
						      &remainder);
				fvco = (u32)(div_u64_rem(freq_hz, 1000000,
							 &remainder));
				if ((fvco >= FVCO_MIN) && (fvco <= FVCO_MAX)) {
					freq_hz = div_u64_rem(freq_hz, dout,
							      &remainder);
	...
	u32 c_osc_clk_freq_hz, u32 *regs_out)
{
	...
	for (i = 0; i < LOGICLK_OUTPUTS; i++)
		clkout_divide[i] = calc_pll_div(c_osc_clk_freq_hz,
						freq_out->freq_out_hz[i],
						clkfbout_mult, divclk_divide);

	for (i = 0; i < LOGICLK_OUTPUTS; i++)
		clkout[i] = calc_pll_count(clkout_divide[i], clkout_phase,
					   clkout_duty);
	...
	regs_out[17] = (get_bits(lock, 34, 30) << 10) | get_bits(lock, 9, 0);
	regs_out[18] = (get_bits(lock, 39, 35) << 10) | get_bits(lock, 19, 10);
